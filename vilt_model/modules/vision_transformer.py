...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # 将mask中为1的地方置为True，然后将attn中与mask中False对应的位置，置为-inf。因为图片mask为全1矩阵，所以对图片无影响。
        
        if mask is not None:
            ipdb.set_trace()
            mask = mask.bool()
            attn = attn.masked_fill(~mask[:, None, None, :], float("-inf"))  # 注意取反符号
...
class VisionTransformer(nn.Module):
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        _, _, ph, pw = self.patch_embed.proj.weight.shape
        #         ipdb.set_trace()
        x = self.patch_embed(_x)  # 64 768 12 12
        #         _x.sum(dim=1) rgb sum; make the not black pixels 1
        x_mask = (_x.sum(dim=1) != 0).float()[:, None, :, :]  # 64 1 384 384 全1
        x_mask = F.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long()  # 64 1 12 12 全1
        #         x_mask[:,0]64 12 12 ; x_mask[:, 0].sum(dim=1)64 12 ; x_mask[:, 0].sum(dim=1)[:, 0] 64

        #         x_h = x_mask[:, 0].sum(dim=1)[:, 0]#64  the num of 1 in the first row
        #         x_w = x_mask[:, 0].sum(dim=2)[:, 0]#64  the num of 1 in the first line

        B, C, H, W = x.shape
        spatial_pos = (  # 1 768  12 12
            self.pos_embed[:, 1:, :]
                .transpose(1, 2)
                .view(1, C, self.patch_dim, self.patch_dim)
        )
        #         ipdb.set_trace()
        #       When training the third batch, a bug appears here___ljx
        #         pos_embed = torch.cat(#64 768 12 12
        #             [
        #                 F.pad(
        #                     F.interpolate(
        ...
        pos_embed = torch.cat(  # 将位置信息重复64次
            [
                F.interpolate(
        ...
        pos_embed = pos_embed.flatten(2).transpose(1, 2)  # 64 144 768
        x = x.flatten(2).transpose(1, 2)  # 64 144 768
        patch_index = (  # 64 144 2 二维绝对坐标
            torch.stack(
                torch.meshgrid(
        ...
        x_mask = x_mask.flatten(1)  # 64 144
        ...
        if (
            ...
            eff = x_h * x_w  # 144
            #             max_image_len = eff.max()#144
            max_image_len = eff
        else:
            eff = x_h * x_w
            #             max_image_len = min(eff.max(), max_image_len)
            max_image_len = min(eff, max_image_len)
        # 为啥要设置这些有效坐标和无效坐标
        valid_idx = x_mask.nonzero(as_tuple=False)  # 9209 2为mask中非零位置的坐标
        non_valid_idx = (1 - x_mask).nonzero(as_tuple=False)  # 7 2
        unique_rows = valid_idx[:, 0].unique()  # 64 从0到63
        valid_row_idx = [valid_idx[valid_idx[:, 0] == u] for u in unique_rows]  # 64 144 2的二维坐标
        non_valid_row_idx = [
            non_valid_idx[non_valid_idx[:, 0] == u] for u in unique_rows
        ]

        valid_nums = [v.size(0) for v in valid_row_idx]  # 64
        non_valid_nums = [v.size(0) for v in non_valid_row_idx]  # 64
        pad_nums = [max_image_len - v for v in valid_nums]  # 64

        select = list()
        for i, (v, nv, p) in enumerate(zip(valid_nums, non_valid_nums, pad_nums)):
            if p <= 0:
                valid_choice = torch.multinomial(torch.ones(v).float(), max_image_len)  # 144
        ...
        select = torch.cat(select, dim=0)  # 9216 2
        #         ipdb.set_trace()
        #        what happens to x and patch_index here___ljx
        x = x[select[:, 0], select[:, 1]].view(B, -1, C)  # 64 144 768-》9216 768-》64 144 768
