...
if opt.stage==1 and opt.img_net=='vilt':
    ...
    from vilt_model.model_text_wyq import ViLTransformerSS, VilT_Classification
...
#
# elif opt.stage==5 and (opt.img_net=='vilt' or opt.img_net=='gru'):
#     from vilt_model.model_up_Cascausal import BaseboneModel, VilT_Classification
    
# elif opt.stage==6 and (opt.img_net=='vilt' or opt.img_net=='gru'):
#     from vilt_model.model_reverse_learning import BaseboneModel, VilT_Classification

elif opt.stage==6 and opt.img_net=='vilt':
    from vilt_model.model_cvm_vilt import BaseboneModel, VilT_Classification

elif opt.stage==6 and opt.img_net=='gru':
    from vilt_model.model_cvm_gru import BaseboneModel, VilT_Classification
    
elif opt.stage==7 and opt.img_net=='vilt':
    from vilt_model.model_cvm_vilt_PP import BaseboneModel, VilT_Classification

elif opt.stage==7 and opt.img_net=='gru':
    from vilt_model.model_cvm_gru_PP import BaseboneModel, VilT_Classification
    
elif opt.stage==1 and opt.img_net=='gru':
    from vilt_model.model_s3d_wyq import gru_encoder_t,VilT_Classification #取相同的名字但是用的是GRU网络
elif opt.stage==2 and opt.img_net=='gru':
    from vilt_model.model_text_gru import gru_encoder_t,VilT_Classification
elif opt.stage==3 and opt.img_net=='gru':
    from vilt_model.model_full_gru import gru_encoder_t,VilT_Classification
# elif opt.stage==4 and opt.img_net=='gru':
#     from vilt_model.model_causal_gru import gru_encoder_t,VilT_Classification
...
opt.dataset = 'msrvtt'
opt.root_path = './msrvtt_train_test/'  # path to root folder
opt.data_path ='./msrvtt_train_test/'
...
if opt.stage in [1,2,3,4,5,6,7] and opt.img_net=='vilt':
    optimizer = optim.Adam(model.parameters(), weight_decay=opt.weight_decay, lr=opt.lr_m1)

elif opt.stage in [1,2,3,4,5,6,7] and opt.img_net=='gru': 
     optimizer = optim.Adam(model.parameters(), weight_decay=opt.weight_decay, lr=opt.lr_m1)
    #ipdb.set_trace()
...
class NTXentLoss(nn.Module):#不加标签信息的loss
    def __init__(self, temperature=0.1, eps=1e-6):
        ...
        self.dense2=nn.Linear(1024,768).cuda()
...
# r 一直运行到return
    ...
    for batch_idx, (data,label) in enumerate(train_loader):
        ...
        if train_stage==1:
            ...
            batch_size_cur=data.size(0)
            if CUDA:
                text_features=data.cuda()
                label=label.cuda()
    
        elif train_stage in [3,6]:
            batch_size_cur=data[0].size(0) #64
          
            if CUDA:
                [s3d_features,text_features]=data
                s3d_features=s3d_features.cuda()
                
                text_features=text_features.cuda()
                label=label.cuda()
#          因果选择模型阶段       
        elif train_stage in [4,5,7]:
#             ipdb.set_trace()
            batch_size_cur = data[0][0].size(0)*2 #当前是32
#             batch_size_cur = data[0][0].size(0)#当前是32

            if CUDA:
                #取出A的正样本为B
                s3d_features=data[0][0]
                text_features=data[0][1]
                index=data[0][2]
                label_A=label[0]
                
                pos_s3d_features=data[1][0]
                pos_text_features=data[1][1]
                pos_index=data[1][2]
                pos_label_A=label[1]
                ...
                pos_text_features=pos_text_features.cuda()
...
#             ipdb.set_trace()
            #帧选择2--采用attention机制
            #output=model(text_features)
            output=model(s3d_features,text_features)
            criterion = nn.CrossEntropyLoss()
            loss_cls = criterion(output, label)
            final_loss = loss_cls
            ...
            if opt.contrastive==True:
                ...
                loss_s2s = criterion_con(text_features, pos_text_features) #计算语义的对比学习
                image_feats,text_feats,output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur)
                #输入正样本
                ...
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
                final_loss = loss_cls+lossAB+0.01*loss_s2s+loss_align  #计算整个的loss

            #部分异构对齐损失
            
            else:
                output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur) #输入正样本
                # compute loss
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
...
#                 output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur) #输入正样本
                ...
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
                final_loss = loss_cls+lossAB+0.01*loss_s2s+0.01*loss_align #计算整个的loss
            else:
                output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur) #输入正样本
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
...
#             ipdb.set_trace()
#             output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur) #输入正样本
            output = model(s3d_features,text_features) #输入正样本
            criterion = nn.CrossEntropyLoss()
            loss_cls = criterion(output, label)
            final_loss = loss_cls #计算整个的loss
        
        elif train_stage==7:
            
            if opt.contrastive==True:
                output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur)
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
                final_loss=loss_cls+lossAB
            else:
                output,lossAB = model(s3d_features,text_features,pos_s3d_features,pos_text_features,batch_size_cur) #输入正样本
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
...
#         optimizer_m2.step()
        ...
        if train_stage in [1,2,3,4,5,6,7]:
            if opt.img_net=='gru':
...
def test_epoch(epoch,stage,test_log):
    ...
    losses_cls1 = AverageMeter('Loss', ':.4e')
    losses_cls2 = AverageMeter('Loss', ':.4e')
    class_correct = [0. for _ in range(20)]
    total_correct = [0. for _ in range(20)]
    class_acc = {}
    model.eval()
    start_time = time.time()
    with torch.no_grad():
...
#                 batch_size_cur = data.item[0].size(0)
                [s3d_features,text_features] = data
                if CUDA:
                    s3d_features= s3d_features.cuda()
                    text_features = text_features.cuda()
                ...
                # full
                output = model(s3d_features, text_features)
#                 image_feats,text_feats,output = model(s3d_features,text_features)
                # compute loss
                criterion = nn.CrossEntropyLoss()
                loss_cls = criterion(output, label)
                acc1, acc5 = accuracy(output, label, topk=(1, 5))
...
#             optimizer.step()
        ...
        if stage in [1,2,3,4,5,6,7]:
            ...
            test_log.flush()
            return top1.avg
            # return image_embeds_pt, embeds_att_pt, embeds_jh_pt ,t_embeds_pt,t_embeds_att_pt,t_embeds_jh_pt
